...
// covariances for x1 (input points) with different m_noise_ params for inputs
EMatrixX
Matern32SparseDeriv1(EMatrixX const& x_1, std::vector<double> gradflag,  // FIXME: sigma of output m_noise_ should also be considered!
    double scale_param, EVectorX const& sigx,
    double const& sigy, EVectorX const& siggrad);

// covariances for x_1 (input points) and x2 (test points)
EMatrixX
Matern32SparseDeriv1(EMatrixX const& x_1, std::vector<double> gradflag,
                                          EMatrixX const& x_2, double scale_param);
...
// covariances for x1 (input points) with a constanct m_noise_ m_param_
EMatrixX
