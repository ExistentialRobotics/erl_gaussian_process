...
EMatrixX
OrnsteinUhlenbeck(EMatrixX const &x1, double scale_param, double sigx) {
    auto n = x1.cols();
...
EMatrixX
OrnsteinUhlenbeck(EMatrixX const &x1, double scale_param, EVectorX const &sigx) {
    auto n = x1.cols();
    double a = 1.f / scale_param;
    EMatrixX K = EMatrixX::Zero(n, n);
...
EMatrixX
Matern32SparseDeriv1(EMatrixX const &x1, std::vector<double> gradflag, double scale_param, EVectorX const &sigx, double const &sigy, EVectorX const &siggrad) {
    auto dim = x1.rows();

    EMatrixX K;

    if (dim == 2) K = matern32_sparse_deriv1_2D(x1, gradflag, scale_param, sigx, sigy, siggrad);
    else if (dim == 3)
        K = matern32_sparse_deriv1_3D(x1, gradflag, scale_param, sigx, sigy, siggrad);

    return K;
}

EMatrixX
Matern32SparseDeriv1(EMatrixX const &x1, std::vector<double> gradflag, EMatrixX const &x2, double scale_param) {
    auto dim = x1.rows();
